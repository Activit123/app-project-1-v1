...
export class MemberAssignmentService {
  ...
  deallocationProposal(managerUserId: string, projectId: string, reason: string): Observable<any> {
    ...
    const url = `${this.apiUrl}/AssignmentProposal?managerUserId=${managerUserId}&projectId=${projectId}&reason=${reason}`;
    return this.http.post<any>(url,null);
  }

  updateDeleteDeallocationProposal(managerUserId: string, proposalId: string, reason: string, type: boolean): Observable<any> {
    const url = `${this.apiUrl}/AssignmentProposal?managerUserId=${managerUserId}&proposalId=${proposalId}&reason=${reason}&type=${type}`;
    return this.http.post<any>(url,null);
  }
}
